    ...
    ui(new Ui::MainWindow)
{
    ...
    QPixmap pm(":/assets/img/infoImage.jpg"); //seats info image
...
void MainWindow::updateUI()  //main window design
{
    qDebug() << "in updateUI()";

    QSqlQuery query(MyDB::getInstance()->getDBInstance());
    query.prepare("select Seat, Available from cppbuzz_movie_ticket_booking");  //executing query
    ...
    while(query.next())
    {
        ...
        {   qDebug() << "label found "<<sLabel;
            if(query.value(1).toString().compare("N") == 0)
            {
                ...
                QPixmap pm(":/assets/img/booked.jpg");  //booked seats image
                ptr->setPixmap(pm);
            ...
            {
                QPixmap pm(":/assets/img/available.jpg");  //available seats image
                ptr->setPixmap(pm);
                ptr->setScaledContents(true);
...
void MainWindow::on_btnBook_clicked()  //book button
{
    ptrBook->updateUI();
    ptrBook->show();
}

void MainWindow::on_btnCancel_clicked()   //cancel button
{
    ptrCancel->updateUI();
    ptrCancel->show();
}
...
void MainWindow::on_btnReset_clicked()
{
    ...
    QSqlQuery query(MyDB::getInstance()->getDBInstance());
    query.prepare("update cppbuzz_movie_ticket_booking set Available = 'Y'");  //executing query
    ...
    if(!query.exec())
    {
        ...
        if(query.numRowsAffected() > 0)
        {
            ...
            qDebug() << "Update was successful. "<< query.lastQuery();  //success
        }
        else
        {
