...
void book::updateUI()
{
    ...
    query.prepare("select Seat from cppbuzz_movie_ticket_booking where Available='Y'"); //executing query
...
void book::on_btnBook_clicked()
{
    ...
    QSqlQuery query(MyDB::getInstance()->getDBInstance());
    query.prepare("update cppbuzz_movie_ticket_booking set Available='N' where Seat= '" + sSeat +"'");  //executing query
