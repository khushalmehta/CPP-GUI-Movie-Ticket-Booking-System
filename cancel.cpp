...
void cancel::updateUI()
{
    ...
    query.prepare("select Seat from cppbuzz_movie_ticket_booking where Available='N'"); //executing query
...
void cancel::on_btnCancel_clicked()
{
    ...
    QSqlQuery query(MyDB::getInstance()->getDBInstance());
    query.prepare("update cppbuzz_movie_ticket_booking set Available='Y' where Seat= '" + sSeat +"'"); //executing query
